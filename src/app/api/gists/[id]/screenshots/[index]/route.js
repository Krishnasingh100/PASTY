...
export async function GET(req, { params }) {
  try {
    ...
    const shots = await db
      ...
      .where(and(eq(gistAttachments.gistId, live[0].id), eq(gistAttachments.kind, "screenshot")))
      .orderBy(asc(gistAttachments.id));
    const shot = shots[idx];
    if (!shot) {
      return Response.json({ success: false, message: "Screenshot not found" }, { status: 404 });
    }
    return new Response(Buffer.from(shot.dataBase64, "base64"), {
      headers: { "Content-Type": shot.mime, "Content-Length": String(shot.size), "Cache-Control": "public, max-age=86400" },
    });
  } catch (e) {
